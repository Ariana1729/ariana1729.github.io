...
E = Matrix(GF(p),E)
LUL = Matrix(GF(p),LUL)
ULUL = Matrix(GF(p),ULUL)
RiS8 = Matrix(GF(p),RiS8)

U = ULUL*LUL^-1
S2 = U^-1*ULUL*U
R = S2^4*RiS8^-1

e = lcm(i[0].degree() for i in factor(S2.characteristic_polynomial()))
S2 = S2.change_ring(GF(p^e))
a,b = S2.eigenmatrix_left()

assert S2 == b^-1*a*b

a12 = [i.sqrt() for i in a.diagonal()]

alphabet = '=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$!?_{}<>'
for sgn in cartesian_product([[-1,1]]*len(a12)):
    S = b^-1*diagonal_matrix([i*j for i,j in zip(a12,sgn)])*b
    if all(all(i.minpoly().degree()==1 for i in j) for j in S):
        L = S*U^-1
        Y = L*E
        X = S^-1*Y
        A = X - R
